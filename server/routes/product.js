...
router.post('/', (req, res) => {
    ...
    const product = new Product(req.body) // models/Product.js 
...
router.post('/products', (req, res) => {
    ...
    // 정렬 순서 설정 참고: https://www.inflearn.com/questions/143367/%EB%9E%9C%EB%94%A9-%ED%8E%98%EC%9D%B4%EC%A7%80%EC%97%90-%EC%83%81%ED%92%88%EB%93%A4%EC%9D%84-%EC%A0%95%EB%A0%AC%ED%95%98%EB%8A%94-%EB%B0%A9%EB%B2%95
    ...
    // filters : LandingPage.js에서 showFIlteredResults에 이용됨
    // filters의 key : restaurantTypes, price
    for (let key in req.body.filters) {
        ...
        if (req.body.filters[key].length > 0) {
            ...
            if (key === "price") {
                findArgs[key] = {
                    // LandingPage/Sections/Datas.js의 array에서 0~199라면 0과 199를 포함한 사이의 값을 넣기 위해 아래의 코드 사용
                    //Greater than equal
                ...
                findArgs[key] = req.body.filters[key];  // restauranttype인 경우 체크박스를 누른 것들을 findArgs에 넣기
            }
    ...
    if (term) {
        ...
        Product.find(findArgs)
            // 이전코드: .find({ $text: { $search: term } })  아래의 else구문과 달리 .find가 하나 더 들어감! LandingPage구현하는 강의 맨 마지막꺼 7분 참고
...
// DetailProductPage에서 get request를 요청하면, 여기서 처리
